/**
 * US-013 — Workspace completion panel (Cap. 7).
 *
 * - Marcar trabajo como procesado en cualquier momento (con o sin habitaciones pendientes).
 * - Reabrir trabajo cerrado para seguir procesando habitaciones.
...
export function RoomProcessingPanel({
  ...
  const isClosed = jobStatus === 'procesado'
  ...
  async function handleReopen() {
    ...
    setReopening(true)
    try {
      const res = await fetch(`${apiBase}/api/jobs/${encodeURIComponent(jobId)}/workspace/reopen`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${accessToken}`,
          'Content-Type': 'application/json',
      ...
      setError('Error de red al reabrir el trabajo.')
    } finally {
      setReopening(false)
    }
  }

  if (!canMarkComplete && !isClosed) return null
      ...
      <div className="mb-6 overflow-hidden rounded-xl border border-outline-variant bg-surface-container-lowest shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-4 border-b border-outline-variant bg-surface-container-low/30 px-6 py-4">
          <div>
            ...
            <p className="mt-1 text-body-sm text-on-surface-variant">
              Este trabajo está marcado como procesado. Podés seguir editando vía chat o reabrirlo para
              procesar más habitaciones.
            </p>
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-4 px-6 py-4">
          <button
            ...
            disabled={reopening}
            className="ml-auto inline-flex items-center gap-2 rounded-lg border border-primary bg-primary/10 px-4 py-2 text-body-sm font-semibold text-primary transition-colors hover:bg-primary/20 disabled:cursor-not-allowed disabled:opacity-50"
          >
            <Icon name="lock_open" className="text-[16px]" />
            {reopening ? 'Reabriendo…' : 'Reabrir trabajo'}
          </button>
        </div>
        {error ? (
    ...
    <div className="mb-6 overflow-hidden rounded-xl border border-outline-variant bg-surface-container-lowest shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-outline-variant bg-surface-container-low/30 px-6 py-4">
        <div>
          ...
          <p className="mt-1 text-body-sm text-on-surface-variant">
            Podés marcar el trabajo como procesado en cualquier momento. La descarga del DXF está
            disponible desde la primera habitación procesada.
          </p>
        </div>
        {!normativeRulesEnabled ? (
      ...
      <div className="flex flex-wrap items-center gap-4 px-6 py-4">
        {rooms.length > 0 && pendingCount > 0 ? (
          <p className="text-body-sm text-on-surface-variant">
            {pendingCount} habitación{pendingCount !== 1 ? 'es' : ''} aún pendiente
            {pendingCount !== 1 ? 's' : ''} — podés cerrar igual o seguir procesando.
          </p>
        ) : rooms.length > 0 ? (
          <p className="text-body-sm text-success">Todas las habitaciones están procesadas u omitidas.</p>
        ) : (
          <p className="text-body-sm text-on-surface-variant">
            Sin habitaciones detectadas — podés cerrar el trabajo cuando quieras.
          </p>
        )}
